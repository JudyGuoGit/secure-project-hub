...
public class AuditLogController {
    ...
    @Operation(summary = "Get all audit logs", description = "Retrieve a list of all audit logs in the system (requires ADMIN role)")
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Get audit log by ID", description = "Retrieve a specific audit log by its ID (requires ADMIN role)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Audit log found"),
        @ApiResponse(responseCode = "404", description = "Audit log not found"),
        @ApiResponse(responseCode = "403", description = "Forbidden - only ADMIN can view audit logs")
    })
    ...
    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Create new audit log", description = "Create a new audit log entry in the system (requires ADMIN role)")
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "403", description = "Forbidden - only ADMIN can create audit logs")
    })
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Update audit log", description = "Update an existing audit log's information (requires ADMIN role)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Audit log updated successfully"),
        @ApiResponse(responseCode = "404", description = "Audit log not found"),
        @ApiResponse(responseCode = "400", description = "Invalid audit log data"),
        @ApiResponse(responseCode = "403", description = "Forbidden - only ADMIN can update audit logs")
    })
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Delete audit log", description = "Delete an audit log from the system (requires ADMIN role)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "204", description = "Audit log deleted successfully"),
        @ApiResponse(responseCode = "404", description = "Audit log not found"),
        @ApiResponse(responseCode = "403", description = "Forbidden - only ADMIN can delete audit logs")
    })
